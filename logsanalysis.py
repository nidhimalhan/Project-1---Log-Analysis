...
query2 = """select auth.name, count(l.path) as most_views from authors auth, articles a, log l
where a.author = auth.id and l.path like concat('%',a.slug, '%') and status like '%200%'
group by auth.name
order by most_views desc;"""

# Query 3 below utilizes three views created in the db to find the number of days where more that 1% of requests lead to errors
...
# This function finds the number of days when more than 1% of requests lead to errors
...
def error_occured(query):
    ...
    for r in output:
        print('\t' + str(r[0]) + ' ' + '-' + ' ' + str("%.2f" % round(r[1],2)) + '%' + ' ' + 'errors')
